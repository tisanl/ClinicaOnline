...
@Component({
  ...
  imports: [MatTabsModule, MatSortModule, MatButton, DatePipe, NombreCompletoPipe, CapitalizeAllPipe, MatDatepickerModule, MatInputModule, MatFormFieldModule],
...
export class Informes implements OnInit {
  ...
  async onTabChange(event: any) {
    ...
    switch (index) {
      ...
      case 2:
        ...
        this.crearGrafico(this.graficoTurnosPorDiaRef.nativeElement, 'line', this.turnosPorDia.map(d => d.dia), this.turnosPorDia.map(d => d.cantidad));
        break;
    }
  ...
  crearGrafico(canvas: HTMLCanvasElement, tipo: keyof ChartTypeRegistry, labels: string[], valores: number[], usarEscala: boolean = true, stepSize: number = 1): Chart {
    ...
    const opcionesEscala = usarEscala ? { y: { beginAtZero: true, ticks: { stepSize: stepSize } } } : undefined;
